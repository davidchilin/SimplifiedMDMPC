...
import core.util.PEa;
import core.util.PEb;
import core.util.PlotIt6;
import core.util.Sontag;
import core.util.Stuff;

public class SeqMPCControl_FDI5 {
	public static void main(String[] args) {
		...
		double[][] xx = new double[(cc + 1) / dilute][Constants.ODE]; // for plotting
		double[][] u = new double[cc + 1][2 + 5];
		double[][] uu = new double[(cc + 1) / dilute][2 + 5]; // for plotting
		...
		/* all state residuals 
		 int[] rdisplay = new int[Constants.ODE];
		 for (int i = 0; i < rdisplay.length; i++)
		 rdisplay[i] = i;
		 end all state residuals */
		...
		double[][] fxk = new double[Constants.ODE][rdisplay.length]; // current state per filter
		double[][][] fu_prev = new double[N][7][rdisplay.length];
		double[][] fu1_opt = new double[n1][rdisplay.length];
		double[][] fu2_opt = new double[n2][rdisplay.length];
		double[][] fu3_opt = new double[n3][rdisplay.length];
		...
		int[][] isolate = { { 0, 0 }, { 0, 0 } }; // first index r to exceed threshold, second is counter
		int isolated = 10; // minimum time to confirm a problem
		int detected = -1; // fault detection status
		// end fdi initiate ***************************************
		double[] fault = {0}; // value of fault after estimation
		double[][] pex = new double[Constants.ODE][isolated+1]; // x record for paramter estimation
		double[][] peu = new double[7][isolated+1]; // u record for parameter estimation, F4,F6, Q1,2,3,4,5
		double[] Qnew = new double[7]; // F4,F6,Q1,2,3,4,5
		t[0] = 0;
		...
		try {
			...
			for (int i = 0; i < cc; i++) {
				...
				if (detected == -1) {// if not detected - perform FDI
					...
					for (int j : rdisplay) {
						if (r[i][ii] > rlimits[ii++]) { // residual exceed threshold
							...
							if (isolate[0][0] == j)
								isolate[0][1]++;							
							else if (isolate[0][1] == 0) // if isolate[0][1] not set, update
							{
								isolate[0][0] = j;
								isolate[0][1]++;
							}
							else if (isolate[1][0] == j) // if already set check if = previous r and update counter
								isolate[1][1]++;
						...
						if (detected == -1 && (isolate[0][1] > isolated || isolate[1][1] > isolated)) {
							...
							double[] u_L3 = new double[1]; // input constraint vector, lower bounds
							double[] u_U3 = new double[1]; // input constraint vector, upper bounds
							for (int b = 0; b < 1; b++) {
								u_L3[b] = -1;
								u_U3[b] = 1;
							}

							PEa pe = new PEa(1, 1, u_L3, u_U3, g_L, g_U); // control F20
							pe.setH(Constants.h);
							pe.addNumOption("tol", Constants.h);
							pe.addNumOption("dual_inf_tol", Constants.h); 
							pe.addNumOption("acceptable_tol", Constants.h);
							pe.addIntOption("max_iter", 1500); // maximum iteration number
							pe.addIntOption("print_level", -2); // do not print on screen
							pe.addStrOption("hessian_approximation", "limited-memory");
							pe.setQc(Qc);
							pe.setRc(Rc);
							pe.setRc2(Rc2);
							pe.setXset(xset); // set set-point
							pe.setFarpe(pex);
							pe.setFrca(peu);
							pe.setCounter(isolated);
							pe.setFail(detected);
							fault=pe.solve(fault);
							System.out.print("Fault magnitude : " + fault[0] + "\n");
						
							double[] peu_L3 = new double[7]; // input constraint vector, lower bounds
							double[] peu_U3 = new double[7]; // input constraint vector, upper bounds
							for (int b = 0; b < 7; b++) {
								peu_L3[b] = -1;
								peu_U3[b] = 1;
							}
							
							peu_L3[detected]=fault[0]; // constrain faulty input to fault value
							peu_U3[detected]=fault[0];
							
							PEb peb = new PEb(7, 1, peu_L3, peu_U3, g_L, g_U); // control F20
							peb.setH(Constants.h);
							peb.addNumOption("tol", Constants.h);
							peb.addNumOption("dual_inf_tol", Constants.h); 
							peb.addNumOption("acceptable_tol", Constants.h);
							peb.addIntOption("max_iter", 1500); // maximum iteration number
							peb.addIntOption("print_level", -2); // do not print on screen
							peb.addStrOption("hessian_approximation", "limited-memory");
							peb.setQc(Qc);
							peb.setRc(Rc);
							peb.setRc2(Rc2);
							peb.setXset(xset);
														
							if(detected<5) Qnew[detected+2]=fault[0];
							else if(detected==5) Qnew[0]=fault[0];
							else Qnew[1]=fault[0];
							
							System.out.print("Qbefore : ");
							
							for(int yy=0;yy<7;yy++)
								System.out.print(yy + ": " + Qnew[yy] + ", ");
							
							Qnew=peb.solve(Qnew); // solve for new steady state input values
							
							System.out.print("\n Qafter : ");
							for(int yy=0;yy<7;yy++)
								System.out.print(yy + ": " + Qnew[yy] + ", ");
							
							// set dyn to proper new steady state values, must scale with umax!!!
							dyn.setF4(Qnew[0]*Constants.umax[0]);
							dyn.setF6(Qnew[1]*Constants.umax[1]);
							double[] Q5new = {Constants.Q2[0]+Qnew[2]*Constants.umax[2],Constants.Q2[1]+Qnew[3]*Constants.umax[3],Constants.Q2[2]+Qnew[4]*Constants.umax[4],Constants.Q2[3]+Qnew[5]*Constants.umax[5],Constants.Q2[4]+Qnew[6]*Constants.umax[6]};
							dyn.setQ(Q5new);
					...
					// keep record for pe
					if (isolate[0][1] != 0) {
						Stuff.Equal(xk, pex, isolate[0][1]-1);

						for (int k = 0; k < 2; k++) {
							peu[k][isolate[0][1]-1] = uk_f[k];
						}
						for (int k = 0; k < 5; k++) {
							peu[k+2][isolate[0][1]-1] = uk[k];
						}
					}
				} // end fault detection
				...
				if (i % nc == 0) {
					...
					for (int z = 0; z < rdisplay.length; z++) { // fdi
						Stuff.Equal(son.predict(Stuff.Equal(fxk, z), N, nc),
								fuk_prev, z); // fdi
					}
					...
					for (int z = 0; z < rdisplay.length; z++)
						Stuff.Equal(fmpc1[z].solve(Stuff.Equal(fu10, z)),
								fu1_opt, z); // fdi
					...
					for (int z = 0; z < rdisplay.length; z++)
						// fdi
						Stuff.Equal(fmpc2[z].solve(Stuff.Equal(fu20, z)),
								fu2_opt, z); // fdi
					...
					for (int z = 0; z < rdisplay.length; z++)
						// fdi
						Stuff.Equal(fmpc3[z].solve(Stuff.Equal(fu30, z)),
								fu3_opt, z); // fdi
				...
				if (detected == -1) { // if fault is not isolated, continue with
					if (i > cc / 2) { // initiate actuator fault here ^^^
						if (i - 2 < cc / 2)
							System.out.print("Fault Triggered at:" + i + "\n");

						uk_f[0] = -.99; // F4, F6
						//uk[1]=0.5; // Q1, Q2, Q3, Q4, Q5
					}
				}
				// initiate reconfigration here
				else if (detected != -2) {// if detected is set to faulty actuator, stop fdi
					switch (detected) {
					// mpc2
					case 0:
						for (int iii = 0; iii < N; iii++) {
							u2_L[iii * nn2 + 0] = fault[0];
							u2_U[iii * nn2 + 0] = fault[0];
						}
						mpc2 = new DMPC(n2, m, u2_L, u2_U, g_L, g_U); // Q1
						// uk[0,1,2]
						break;
					case 1:
						for (int iii = 0; iii < N; iii++) {
							u2_L[iii * nn2 + 1] = fault[0];
							u2_U[iii * nn2 + 1] = fault[0];
						}
						mpc2 = new DMPC(n2, m, u2_L, u2_U, g_L, g_U); // Q2
						// uk[0,1,2]
						break;
					case 2:
						for (int iii = 0; iii < N; iii++) {
							u2_L[iii * nn2 + 2] = fault[0];
							u2_U[iii * nn2 + 2] = fault[0];
						}
						mpc2 = new DMPC(n2, m, u2_L, u2_U, g_L, g_U); // Q3
						// uk[0,1,2]
						break;
						// mpc3
					case 3:
						for (int iii = 0; i < N; iii++) {
							u3_L[iii * nn3 + 0] = fault[0];
							u3_U[iii * nn3 + 0] = fault[0];
						}
						mpc3 = new DMPC(n3, m, u3_L, u3_U, g_L, g_U); // Q4
						// uk[3,4]
						break;
					case 4:
						for (int iii = 0; i < N; iii++) {
							u3_L[iii * nn3 + 1] = fault[0];
							u3_U[iii * nn3 + 1] = fault[0];
						}
						mpc3 = new DMPC(n3, m, u3_L, u3_U, g_L, g_U); // Q5
						// uk[3,4]
						break;
						// mpc1
					case 5:
						for (int iii = 0; iii < N; iii++) {
							u1_L[iii * nn1 + 0] = fault[0];
							u1_U[iii * nn1 + 0] = fault[0];
						}
						mpc1 = new DMPC(n1, m, u1_L, u1_U, g_L, g_U); // F4
						// uk_f[0,1]
						break;
					case 6:
						for (int iii = 0; iii < N; iii++) {
							u1_L[iii * nn1 + 1] = fault[0];
							u1_U[iii * nn1 + 1] = fault[0];
						}
						mpc1 = new DMPC(n1, m, u1_L, u1_U, g_L, g_U); // F6
						// uk_f[0,1]
						break;
					default:
						detected = 7;
					}
					System.out.print("Fault isolated/reconfigured: "
							+ act_name[detected] + "\n");
					detected = -2;
					...
					mpc1.addIntOption("print_level", -2);
					mpc1
					.addStrOption("hessian_approximation",
					"limited-memory");
					mpc1.addNumOption("tol", step);
					mpc1.addNumOption("dual_inf_tol", step);
					mpc1.addNumOption("acceptable_tol", step);
				...
				for (int z = 0; z < rdisplay.length; z++)
					// fdi
					Stuff.Equal(dyn.solve(Stuff.Equal(fxk, z), Stuff.Equal(fuk,
							z), Stuff.Equal(fuk_f, z)), fxk, z); // fdi
...
//			}
			...
			double[] threshold = new double[rdisplay.length]; // residual
			// threshold
			...
			new PlotIt6("Temperatures", Stuff.Reverse(xx), tt, xset, tdisplay,
					1).plot();
			new PlotIt6("Control Inputs", Stuff.Reverse(uu), tt, udisplay, 1)
			.plot();
			// new PlotIt6("fTemperatures", f1x, t, display,1).plot(); // fdi
			// new PlotIt6("fControl Inputs", f1u, t, udisplay,1).plot(); // fdi
			new PlotIt6("Residuals", Stuff.Reverse(rr), tt, threshold, abc)
			.plot();
