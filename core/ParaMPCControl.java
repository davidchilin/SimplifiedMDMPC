...
public class ParaMPCControl {
	
	public static void main(String[] args){
		ParaMPCControl para = new ParaMPCControl();
		for(int i=1; i<=1; i+=2){
			para.run(5, 1, i);
		}
	}
	
	public void run(int nc, int N, int ite){
		...
		String xfile = "./data/newsim2/para_x_ite"+ite+"_n"+N+"_nc"+nc+".data";
		String ufile = "./data/newsim2/para_u_ite"+ite+"_n"+N+"_nc"+nc+".data";
			...
			for(int i=0; i<cc; i++){
				if(i%nc == 0){
					...
					for(int p=0; p<ite; p++){
						...
						if(p==0)
							System.out.print("At " + i + "\t ite #"+p+": status of MPC 1 is ");
						else
						...
						cet = cet + 1;
						for(int s=0; s<N; s++){
							u_prev[s][0] = u1_opt[0+nn1*s];
							u_prev[s][1] = u1_opt[1+nn1*s];
							u_prev[s][2] = u2_opt[0+nn2*s];
							u_prev[s][3] = u2_opt[1+nn2*s];
							u_prev[s][4] = u2_opt[2+nn2*s];
